...
class ChatRoomViewController: UIViewController {
    ...
    private var safeAreaButton: CGFloat {
        self.view.safeAreaInsets.bottom
    }
    
    //  入力したコメントが反映(表示)されるための変数
    private var messages = [Message]()
    //  チャットコメント入力欄のインスタン生成
    private lazy var chatInputAccessoryView: ChatInputAccessoryView = {
        let view = ChatInputAccessoryView()
        view.frame = .init(x: 0, y: 0, width: view.frame.width, height: accessoryHeight)
        //  ④デリゲートを他にも渡す処理(上記の private var の間に lazy を付け足す。)
        view.delegate = self
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        ...
        if let keyboardFrame = (userInfo[UIResponder.keyboardFrameBeginUserInfoKey] as AnyObject).cgRectValue {
            if keyboardFrame.height <= accessoryHeight { return }
            
            let top = keyboardFrame.height - safeAreaButton
...
//  ⑤デリゲートを他にも渡す処理(コメント入力情報)
extension ChatRoomViewController: ChatInputAccessoryViewDelegate {
    //  コメント入力して送信ボタンを押すと入力したコメントが反映(表示)される。
